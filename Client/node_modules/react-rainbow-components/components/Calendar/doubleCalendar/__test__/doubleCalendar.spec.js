...
describe('DoubleCalendar', function () {
  ...
  it('should change right month year when left month year changed', function () {
    ...
    var leftYearSelect = component.find('YearSelect').at(0).find('select');
    leftYearSelect.simulate('change', {
      target: {
        value: 2020
      }
    });
    expect(component.find('YearSelect').at(0).prop('currentYear')).toBe(2020);
    expect(component.find('YearSelect').at(1).prop('currentYear')).toBe(2021);
  });
  it('should change left month year when right month year changed', function () {
    ...
    component.find('YearSelect').at(1).find('select').simulate('change', {
      target: {
        value: 2023
      }
    });
    expect(component.find('YearSelect').at(0).prop('currentYear')).toBe(2022);
    expect(component.find('YearSelect').at(1).prop('currentYear')).toBe(2023);
  });
});